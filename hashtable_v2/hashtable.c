...
/* function to create a new hashtable with size = 8. */
hashtable *new_hashtable(){
          return create_hashtable(8);         
}
...
/* insert a key-value pair into a hash table without resizing  */
...
/* inset a key_value pair with resizing */
void insert_resize_pair(hashtable **hash_table, char *key, char *value ){
     if (3*cases_full(*hash_table)>=2*(*hash_table)->size) {
          *hash_table=resize_hashtable(*hash_table);
     }
     insert_pair(*hash_table,key,value);
}
...
hashtable *resize_hashtable(hashtable *hash_table){
    int i;
    entry * list;
    hashtable *new_hash_table=create_hashtable(2*hash_table->size);       
    for(i=0; i<hash_table->size; i++) {
        list = hash_table->table[i];
        while(list!=NULL) {
            insert_pair(new_hash_table,list->key,list->value);
            list=list->next;
        }  
    } 
