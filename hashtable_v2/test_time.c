...
int main( int argc, char **argv ) {
	...
	for (j=0;j<(int) (sizeof(N)/sizeof(int));j++){
        ...
        printf("\n\n############################# The time test N_keys= %d #################################\n\n",N[j]);
        printf ("The opertion for the element number i :  Insertion \n");
         start=clock();
	    for(i=0;i<N[j];i++) {
                     insert_resize_pair( &hash_table, keys[i], values[i] );
                     stop=clock();
                     if (N[j]<=N_LINES||i%(N[j]/N_LINES)==0) printf("SIZE= %d \tfor i=%d\t\t the insertion takes %f CPU clocks\n",hash_table->size,i,(double) (stop-start)/(i+1));                       
        }  
        ...
        start=clock();
   	    for(i=0;i<N[j];i++) {
                     ...
                     stop=clock();
                     if (N[j]<=N_LINES||i%(N[j]/N_LINES)==0) printf("SIZE= %d \tfor i=%d\t\t the looking up takes %f CPU clocks\n",hash_table->size,i,(double) (stop-start)/(i+1));                      
        }                              
